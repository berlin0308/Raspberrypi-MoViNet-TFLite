...
_FONT_SIZE = 1
_FONT_THICKNESS = 1
_MODEL_FPS = 10 # Ensure the input images are fed to the model at this fps.
...
def run(model: str, label: str, max_results: int, num_threads: int,
  ...
  while cap.isOpened():
    ...
    if diff * _MODEL_FPS >= (1 - _MODEL_FPS_ERROR_RANGE):
      ...
      categories, time_per_infer = classifier.classify(frame_rgb)
...
def main():
  ...
  parser.add_argument(
      ...
      required=False,
      default='models/a1_v2_stream_b16_lr0.0001_g1.5_d0.5_918/a1_float16.tflite')
  parser.add_argument(
      '--label',
