...
class VideoClassifier(object):
  ...
  def __init__(
    ...
    Arm NN Delegate
    """
    # armnn_delegate = tflite.load_delegate('./libarmnnDelegate.so', 
    #                                   options={"backends": "CpuAcc, CpuRef", "logging-severity": "info"})

    # interpreter = tflite.Interpreter(model_path=model_path, num_threads=options.num_threads, experimental_delegates=[armnn_delegate])
    ...
    try:
      ...
      signature = interpreter.get_signature_runner('call')
  ...
  def classify(self, frame: np.ndarray) -> Tuple[List[Category], float]:
    ...
    infer_time = time.time()-current_time
